...
public class PoolManager : MonoBehaviour
{
	...
	public void Awake()
	{
		for (int i = 0; i < this.pools.Length; i++)
		{
			...
			for (int j = 0; j < this.pools[i].poolSize; j++)
			{
				...
				this.pools[i].poolObjectList.Add(gameObject2);
			}
		}
	}
	...
	public GameObject GetObject(string poolName, Vector2 pos, Quaternion rot)
	{
		for (int i = 0; i < this.pools.Length; i++)
		{
			...
			{
				for (int j = 0; j < this.pools[i].poolObjectList.Count; j++)
				{
					if (!this.pools[i].poolObjectList[j].activeInHierarchy)
					{
						GameObject gameObject = this.pools[i].poolObjectList[j];
	...
	{
		for (int i = 0; i < this.pools[poolID].poolObjectList.Count; i++)
		{
			if (!this.pools[poolID].poolObjectList[i].activeInHierarchy)
			{
				GameObject gameObject = this.pools[poolID].poolObjectList[i];
				gameObject.transform.position = pos;
				gameObject.transform.rotation = rot;
				gameObject.SetActive(true);
				return gameObject;
		...
		if (!this.pools[poolID].limit)
		{
			...
			gameObject2.transform.position = pos;
			gameObject2.transform.rotation = rot;
			this.pools[poolID].poolObjectList.Add(gameObject2);
			return gameObject2;
		}
		return null;
	}
